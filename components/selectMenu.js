...
class EZSelectMenu {
    constructor(container, options = {}) {
        ...
        this.element = null;
        this.elements = {}; // Will store references to sub-elements
        this.isOpen = false;
    ...
    render() {
        ...
        if (this.options.disabled) {
            this.element.classList.add('ez-select-disabled');
        }

        // Build selected value display
        const selectedOption = this._getSelectedOption();

        // Build main button that displays selected option
        let selectButtonHTML = `
            <div class="ez-select-button">
                ${selectedOption ? this._renderOptionContent(selectedOption) : `
        ...
        if (!document.getElementById('ez-select-styles')) {
            this._addStyles();
        }
    ...
    _renderOptionContent(option) {
        ...
        if (option.emoji) {
            html += `<div class="ez-select-option-emoji">${option.emoji}</div>`;
        }
        ...
        if (option.description) {
            html += `<div class="ez-select-option-description">${option.description}</div>`;
        }
    ...
    setValue(value) {
        ...
        if (this.onChangeCallback && prevValue !== value) {
            this.onChangeCallback(value, this);
        }
    ...
    enable() {
        ...
        if (this.element) {
            this.element.classList.remove('ez-select-disabled');
        }
        return this;
    }
