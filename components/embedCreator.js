...
class EZEmbedCreator {
    constructor(container, options = {}) {
        ...
        // Add a new property to cache the last known state
    ...
    _createDOM() {
        ...
        colorPickerModal.innerHTML = `
            ...
            <div id="color-picker-content" class="color-picker-content">
                ...
                <div class="color-presets">
                    ${this.options.colorPresets.map(color =>
        ...
        if (!document.getElementById('embed-creator-styles')) {
            this._addStyles();
        }
    ...
    _loadInitialImages() {
        ...
        if (this.currentData.thumbnail) {
            this.elements.thumbnail.innerHTML = `<img src="${this.currentData.thumbnail}" style="width:100%; height:100%; object-fit:cover; border-radius:4px;">`;
        }


        if (this.currentData.image) {
            this.elements.imagePanel.innerHTML = `<img src="${this.currentData.image}" style="max-width:100%; max-height:300px; border-radius:4px;">`;
        }


        if (this.currentData.author && this.currentData.author.iconUrl) {
            this.elements.authorIcon.innerHTML = `<img src="${this.currentData.author.iconUrl}" style="width:100%; height:100%; object-fit:cover; border-radius:50%;">`;
        }
    }
    ...
    _updateEmbedColor(color) {
        ...
        if (this.elements.embedColorPicker) {
            this.elements.embedColorPicker.value = color;
        }
    }
    ...
    _getChangedData() {
        ...
        const compareObjects = (lastKnown, current, path = '') => {
            ...
            if (Array.isArray(lastKnown) && Array.isArray(current)) {
                if (JSON.stringify(lastKnown) !== JSON.stringify(current)) {
                    changes[path.slice(1)] = current;
                }
                return;
            }
            ...
            for (const key in lastKnown) {
                if (!current.hasOwnProperty(key)) {
                    changes[`${path}.${key}`.slice(1)] = undefined;
                }
            }
        };
