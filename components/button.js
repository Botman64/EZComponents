...
class EZButton {
    constructor(container, options = {}) {
        ...
        this.options = {
            ...
            style: options.style || 'primary', // primary, secondary, success, danger, link
            emoji: options.emoji || '',
            url: options.url || '',
    ...
    render() {
        ...
        if (this.options.emoji) {
            buttonHTML += `<span class="EZbtn-emoji">${this.options.emoji}</span>`;
        }
        ...
        if (!document.getElementById('EZbtn-styles')) {
            this._addStyles();
        }
    ...
    setEmoji(emoji) {
        ...
        if (this.element) {
            ...
            // If emoji is being removed
            if (!emoji && emojiElement) {
                emojiElement.remove();
                this.elements.emoji = null;
            }
            // If emoji is being added or changed
            else if (emoji) {
                if (emojiElement) {
    ...
    // Get button properties as an object
    toJSON() {
        return {
